...
# Всё есть объект - манипуляции с объектом (примеры)
# a, b = 3, 4
#
# c = a + b # "под капотом" -> a.__add__(b)
# c = a / b  # "под капотом" -> a.__div__(b)
# c = int(a) # "под капотом" -> a.__str__()

class A:
    def __init__(self, a=0):
        self.a = a

    # Переопределяем (override) преобразование в строку
    def __str__(self):
        return f'<Объект класса А со значением а = {self.a}>'

    # Строковое представление (representation) сложного объекта
    def __repr__(self):
        return f'<Объект класса А со значением а = {self.a}>'
